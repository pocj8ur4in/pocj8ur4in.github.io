...
- ```~1996``` : ```HTML``` 문서에 사용자가 원하는 스타일을 적용하고자 함
  - ```Inline Style Sheet``` : ```HTML``` 태그의 ```style``` 속성으로 일일이 적용
...
- ```1996``` : 레이아웃 및 디자인 정보로 인해 ```HTML```의 구조화에 어려움을 겪자 내용과 디자인을 분리하기 위해 ```CSS``` 탄생
  - ```Linking Style Sheet``` : 별도의 스타일을 선언 (```CSS declaration```)하고 태그를 선택 (```CSS selector```)해 적용

```
<link rel="stylesheet" href="style.css">
```

- 문서를 더 잘 표현하기 위해 보다 적은 코드로 같은 내용에 다양한 서식을 적용하도록 발전
- 웹 서비스의 범위가 기존의 문서에서 서비스와 어플리케이션의 형태로 확장되면서 이에 대한 솔루션을 추가
...
> - ```CSS```가 웹 문서가 웹 사이트에 표현하는 데에서 시작되었기에 대규모의 웹 서비스를 만드는 데에 근본적인 한계가 존재
>   - ```CSS```의 한계를 극복하기 위한 최신 기술들을 꾸준히 학습하고 이를 프로젝트에 업데이트해야 함

<img src="https://velog.velcdn.com/images%2Fteo%2Fpost%2Fdaba1ce2-4a19-4e4c-91fd-46423fde95b1%2Fimage.png" width="70%">

### ```CSS``` 스타일시트 (```CSS Style Sheet```) : 웹 브라우저에 특정 ```HTML``` 요소를 렌더링하는 방식 지정
- ```CSS``` 일반 속성 (```CSS Rule Set```) : ```CSS``` 스타일시트에서의 각 구성 요소
...
CSS 선택자 { /* 값은 속성을 여러 번 선언하였다면, 나중에 설정된 값이 적용 : CSS 값2가 적용된다. */
    CSS 속성1 : CSS 값1;
    CSS 속성1 : CSS 값2;
}

CSS 선택자 { /* !important : CSS 값 뒤에 !important가 사용된 것을 우선시 : CSS 값1가 적용된다. */
