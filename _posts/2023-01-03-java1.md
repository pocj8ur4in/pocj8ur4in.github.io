...
### 런타임 데이터 에리어 (```Runtime Data Area```) : 프로그램을 수행하기 위해 운영체제에서 할당받은 메모리 공간
...
- 가비지 컬렉션 (```garbage Collection```) : ```new```로 할당받은 후 사용하지 안게 된 객체의 메모리 공간을 적절한 시점에 자동으로 ```JVM```에 반환
