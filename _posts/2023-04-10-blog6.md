...
적용하는 방법도 html 파일에 아래의 스크립트를 넣어주기만 하면 되는데, 나 같은 경우에는 Minimal Mistakes 테마에서 ```_includes/social_share.html``` 파일에 해당 구문을 넣어 적용시켰다.
...
만약에 블로그의 방문자 수 통계를 보고 싶다면, <a href="https://analytics.google.com/analytics/web/provision/#/provision">Google Analytics</a>를 가입한 후 애널리틱스 데이터 스트림을 설정하면 된다. 이때, 측정 ID를 복사한 후 ```_config.yml``` 파일을 다음과 같이 수정해야 한다.
