...
### HTML 값 (HTML arguments)
- 태그 내의 속성에 해당되는 변수의 값
...
##### ```<meta http-equiv="X-UA-Compatible">``` : 웹 문서를 렌더링할 IE 버전을 명시하여 웹의 호환성을 지정하는 http-equiv 값
...
##### ```<meta http-equiv="content-type">``` : 웹 문서에 대한 웹 브라우저의 문자 인코딩 방식을 지정하는 http-equiv 값
- '호환성 보기'
...
##### ```<meta http-equiv="content-language">``` : 웹 문서 내의 언어를 명시하는 http-equiv 값
...
##### ```<meta http-equiv="content-security-policy">``` : XSS, 데이터 삽입 공격 등을 감지하고 완화하기 위해 웹 문서에 대한 컨텐츠 정책을 명시하는 http-equiv 값
...
##### ```<meta http-equiv="content-script-type">``` : 웹 문서에 사용된 프로그래밍 언어를 명시하는 http-equiv 값
...
##### ```<meta http-equiv="default-style">``` : 웹 문서에 우선적으로 적용할 스타일시트를 지정하는 http-equiv 값
...
##### ```<meta http-equiv="refresh">``` : 웹 문서를 새로고침하는 시간 간격을 지정하는 http-equiv 값
...
##### ```<meta http-equiv="refresh; url=">``` : 웹 문서를 새로고침하는 시간 간격을 지정하는 http-equiv 값
...
##### ```<meta http-equiv="pragma">``` : 현재의 웹 문서를 캐시로 저장할지 여부를 지정하는 http-equiv 값
...
##### ```<meta http-equiv="expires">``` : 현재의 웹 문서의 웹 캐시 만료일을 지정하는 http-equiv 값
...
##### ```<meta http-equiv="page-enter">``` : 현재 웹 문서로 접속할 때의 장면 전환 효과를 지정하는 http-equiv 값

##### ```<meta http-equiv="page-exit">``` : 현재 웹 문서를 종료할 때의 장면 전환 효과를 지정하는 http-equiv 값
...
##### ```<meta http-equiv="origin-trial">``` : 웹 개발자가 웹 문서에 실험단계인 속성을 명시하는 http-equiv 값
...
##### ```align="left"``` : 왼쪽 정렬을 의미하는 align 값
##### ```align="center"``` : 가운데 정렬을 의미하는 align 값
##### ```align="right"``` : 오른쪽 정렬을 의미하는 align 값
##### ```align="justfy"``` : 맞춤 정렬을 의미하는 align 값
...
> 
>> - ###### ```<input type="text">``` : 한 줄의 문자열을 생성하는 type 값(기본값)
>> - ###### ```<input type="password">``` : 비밀번호를 생성하는 type 값
>> - ###### ```<input type="range">``` : 지정한 범위의 숫자를 생성하는 type 값
>> - ###### ```<input type="checkbox">``` : 선택/해제할 수 있는 항목 (중복 가능)을 생성하는 type 값
>> - ###### ```<input type="radio">``` : 선택/해제할 수 있는 항목 (중복 불가)을 생성하는 type 값
>> - ###### ```<input type="submit">``` : 클릭할 경우 입력값을 전송하는 버튼을 생성하는 type 값
>> - ###### ```<input type="reset">``` : 클릭할 경우 입력값을 초기화하는 버튼을 생성하는 type 값
>> - ###### ```<input type="file">``` : 파일 업로드 버튼
>> - ###### ```<input type="hidden">``` : 입력값 없이 폼과 함께 전송해야 하는 정보를 감싸는 type 값 (hidden flied)
...
> - ###### ```<video preload="auto">``` : 웹 문서를 열 때 동영상과 메타데이터 모두 로드하는 preload 값
> - ###### ```<video preload="metadata">``` : 웹 문서를 열 때 메타데이터만 로드하는 preload 값
> - ###### ```<video preload="none">``` : 웹 문서를 열 때 동영상과 메타데이터 모두 로드하지 않는 preload 값
