...
## CPU 스케줄링 (```CPU Scheduling```) : CPU 스케줄러에서 어느 프로세스를 다음에 실행할지 지정하는 작업
...
## 프로세스 간 통신 (```Inter-Process Communication```) : 프로세스들 사이에 서로 데이터를 주고받는 행위
...
- 여러 쓰레드들이 하나의 프로세스 안에 속하면서 자원 공유
